...
/** The stored history-row columns from a validated record, for a given UTC date + provenance
 *  (`source` is 'cron' for the daily live capture, 'backfill' for a reconstructed row). */
function buildHistoryRow(marketId, record, snapshotDate, source) {
  const d = record?.snapshot?.derived ?? {};
  return {
    market_id: marketId,
 ...
 *  (market_id, snapshot_date) — a same-day retry overwrites rather than duplicating. The daily
 *  cron's live capture, tagged source='cron' (authoritative over a backfill for the same day). */
export async function writeHistory(marketId, record, lifecycle = null) {
  const row = buildHistoryRow(marketId, record, new Date().toISOString().slice(0, 10), 'cron');
...
/** Insert ONE backfilled history row for a past UTC date. PRECEDENCE: a plain INSERT that
 *  treats a unique-key conflict as a no-op (returns false) — so a real captured 'cron' row is
 *  NEVER overwritten by a reconstruction. Returns true when the row was newly inserted. */
export async function writeBackfillRow(marketId, snapshotDate, record) {
  const row = buildHistoryRow(marketId, record, snapshotDate, 'backfill');
  const { error } = await db().from('market_history').insert(row);
  if (error) {
    if (error.code === '23505') return false; // (market_id, snapshot_date) exists → cron/prior wins
