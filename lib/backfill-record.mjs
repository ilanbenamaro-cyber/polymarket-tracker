...
/** Cap confidence at MEDIUM + add the historical-backfill reason, and mark the snapshot source
 *  as reconstructed. Mutates and returns the record. */
function markBackfill(record) {
  const d = record.snapshot.derived;
  const c = d.confidence;
  if (c) {
    const tier = TIER_RANK[c.tier] > TIER_RANK.medium ? 'medium' : c.tier;
    const reasons = [...(c.reasons ?? [])];
    if (!reasons.some((r) => /historical backfill/i.test(r))) reasons.push(BACKFILL_REASON);
    d.confidence = { ...c, tier, reasons };
  }
