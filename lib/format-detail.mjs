...
const KNOWN_UNITS = new Set(['T', 'B', 'M']);

/** Derive the ladder's money unit (T/B/M) from the first threshold label's suffix.
 *  Labels look like ">$1.8T" / "$2–2.2T" / ">$28B" / ">$500M". Falls back to 'T'. */
export function unitFromLadder(markets) {
  const label = markets?.[0]?.label ?? '';
  const matches = label.match(/[TBM]/g); // last money-unit letter in the label
  const u = matches?.[matches.length - 1];
  return u && KNOWN_UNITS.has(u) ? u : 'T';
}
