...
export async function computeMarketRecord({ id, prior = null }) {
  ...
  if (shape === 'categorical') {
    // Multi-leg, non-numeric outcomes — surface a friendly message instead of letting the
    // threshold parser throw "Cannot parse threshold" into the UI. 422 ≠ 404, so the add
    // flow shows THIS message (not the generic "not a supported threshold-ladder market").
    const e = new Error('This market uses categorical outcomes — numeric threshold or binary markets only');
    e.code = 422;
    throw e;
  }
  if (shape === 'directional_touch') {
    // Directional-touch (WTI/Silver "(LOW)/(HIGH) hit $X") — not a settlement distribution.
    // Its dedicated touch view lands in I3; until then surface an honest 422 rather than
    // mis-rendering it as a (broken) ladder, which is what the old classifier did.
    const e = new Error('This is a directional-touch market — its touch view is not yet available');
    e.code = 422;
    throw e;
  }
