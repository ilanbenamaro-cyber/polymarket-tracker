...
export async function computeMarketRecord({ id, prior = null }) {
  // Detect kind BEFORE any threshold parsing — fetchMarketMeta throws on a leg whose
  // question has no $threshold (binary OR categorical), so probeLifecycle can't classify.
  const kind = await classifyMarketKind(id);
  if (kind === 'binary') return computeBinaryRecord({ id, prior });
  if (kind === 'categorical') {
