 ...
 *                                             fetched_at, record }
 */
/** Format the rail's headline scalar by market kind: a binary market stores its YES
 *  probability in implied_median (overload, no migration) → render as a percentage;
 *  a ladder renders the median via the same fmtT the detail view uses. */
function headlineDisplay(kind, value) {
  if (value == null) return '—';
  return kind === 'binary' ? `${Math.round(value * 100)}%` : fmtT(value);
}
...
export function assembleScanRows(visibleRows, marketRows, latestRows) {
  ...
  const rows = [...byMarket.values()].map((e) => {
    ...
    return {
      ...
      title: titleById.get(e.market_id) ?? e.market_id, // FK guarantees a markets row; id is a safe last resort
      kind,
      scopes: [...e.scopes],
      ...
      median_display: headlineDisplay(kind, median),
